...
pub const Address = union(AddressFamily) {
    ...
    pub const IPv4 = struct {
        ...
        pub fn init(a: u8, b: u8, c: u8, d: u8) Self {
            return Self{ .value = [4]u8{ a, b, c, d } };
        }
    ...
    pub const IPv6 = struct {
        ...
        pub fn eql(lhs: Self, rhs: Self) bool {
            return std.mem.eql(u8, &lhs.value, &rhs.value) and lhs.scope_id == rhs.scope_id;
        }
        ...
        /// Parse an IPv6 representation according to the canonical format described in
        /// [RFC5952](https://datatracker.ietf.org/doc/html/rfc5952). The "scope ID" (otherwise
        /// known as "zone ID", `<zone_id>`) is intentionally not supported as parsing according to
        /// [RFC6874](https://datatracker.ietf.org/doc/html/rfc6874) is highly platform-specific
        ...
        pub fn parse(string: []const u8) !IPv6 {
            ...
            if ((string[0] == ':' and string[1] != ':') or
                (string[string.len - 2] != ':' and string[string.len - 1] == ':'))
            {
                return error.InvalidFormat;
            }
                ...
                switch (c) {
                    ':' => {
                        ...
                        if (groups[cg_index].len == 0 and
                            (!abbreviation_ending or (i != 1 and i != string.len - 1)))
                        {
                            return error.InvalidFormat;
                        }
            ...
            for (groups, 0..) |group, i| {
                ...
                // First byte exists.
                if (group.len > 2) {
                    ip.value[i * 2] = try std.fmt.parseInt(u8, group[0 .. group.len - 2], 16);
    ...
    pub fn eql(lhs: @This(), rhs: @This()) bool {
        if (@as(AddressFamily, lhs) != @as(AddressFamily, rhs)) return false;
        return switch (lhs) {
...
pub const EndPoint = struct {
    ...
    pub fn parse(string: []const u8) !EndPoint {
        const colon_index = std.mem.indexOfScalar(u8, string, ':') orelse
            return error.InvalidFormat;
    ...
    pub fn format(
        ...
        _ = fmt;
        _ = options;
        try writer.print("{}:{}", .{ value.address, value.port });
    }

    pub fn fromSocketAddress(
        src: *const std.posix.sockaddr,
        size: usize,
    ) !Self {
        switch (src.family) {
            std.posix.AF.INET => {
                if (size < @sizeOf(std.posix.sockaddr.in)) return error.InsufficientBytes;
                const value: *align(4) const std.posix.sockaddr.in = @ptrCast(@alignCast(src));
                return EndPoint{
                    .port = std.mem.bigToNative(u16, value.port),
                    .address = .{
                        .ipv4 = .{ .value = @bitCast(value.addr) },
                    },
                };
            },
            std.posix.AF.INET6 => {
                if (size < @sizeOf(std.posix.sockaddr.in6)) return error.InsufficientBytes;
                const value: *align(4) const std.posix.sockaddr.in6 = @ptrCast(@alignCast(src));
                return EndPoint{
                    .port = std.mem.bigToNative(u16, value.port),
                    .address = .{
                        .ipv6 = .{ .value = value.addr, .scope_id = value.scope_id },
                    },
                };
            },
            else => {
...
/// A network socket, can receive and send data for TCP/UDP and accept incoming connections if
/// bound as a TCP server.
pub const Socket = struct {
    ...
    pub fn create(family: AddressFamily, protocol: Protocol) !Self {
        ...
        // Socket creation will only set the flag if we provide the shim rather
        // than the actual flag.
    ...
    pub fn setWriteTimeout(self: *Self, write: ?u32) !void {
        ...
        try std.posix.setsockopt(
            ...
            if (is_windows)
                ...
                std.mem.toBytes(write_timeout)[0..],
        );
    }

    /// Sets the SO_BROADCAST socket option to enable/disable UDP broadcast.
    ...
    pub fn connect(self: *Self, target: EndPoint) !void {
        if (target.address != self.family) return error.AddressFamilyMismach;
    ...
    /// Makes this socket a TCP server and allows others to connect to this socket.
    /// Call `accept()` to handle incoming connections.
    pub fn listen(self: Self) !void {
        try std.posix.listen(self.internal, 0);
    }

    /// Waits until a new TCP client connects to this socket and accepts the incoming TCP
    /// connection. This function is only allowed for a bound TCP socket. `listen()` must have been
    /// called before!
    ...
    /// Send some data to the connected peer. In UDP, this will always send full packets, on TCP it
    /// will append to the stream.
    pub fn send(self: Self, data: []const u8) SendError!usize {
        if (self.endpoint) |ep| return try self.sendTo(ep, data);
    ...
    /// Will read all available data from the TCP stream or a UDP packet.
    ...
    /// Same as ´receive`, but will also return the end point from which the
    /// data was received. This is only a valid operation on UDP sockets.
    ...
    /// Sends a packet to a given network end point. Behaves the same as `send()`, but will only
    /// work for UDP sockets.
    ...
    /// Sets the socket option `SO_REUSEPORT` which allows multiple bindings of the same socket to
    /// the same address on UDP sockets and allows quicker re-binding of TCP sockets.
...
pub const SocketEvent = struct {
    ...
    /// Wait for all pending data to be sent and the socket accepting non-blocking writes.
    write: bool,
};

/// A set of sockets that can be used to query socket readiness.
/// This is similar to `select()´ or `poll()` and provides a way to create non-blocking socket I/O.
...
pub const SocketSet = struct {
    ...
    /// Adds a socket to the set and enables waiting for any of the events in `events`.
...
const LinuxOSLogic = struct {
    ...
    inline fn init(allocator: std.mem.Allocator) !Self {
        return Self{ .fds = std.ArrayList(std.posix.pollfd).init(allocator) };
    }
    ...
    inline fn add(self: *Self, sock: Socket, events: SocketEvent) !void {
        ...
        if (events.read) mask |= std.posix.POLL.IN;
        if (events.write) mask |= std.posix.POLL.OUT;
        ...
        try self.fds.append(std.posix.pollfd{ .fd = sock.internal, .events = mask, .revents = 0 });
    }

    inline fn remove(self: *Self, sock: Socket) void {
        const index = for (self.fds.items, 0..) |item, i| {
            if (item.fd == sock.internal) break i;
        } else null;
    ...
    inline fn checkMaskAnyBit(self: Self, sock: Socket, mask: i16) bool {
        for (self.fds.items) |item| {
            if (item.fd != sock.internal) continue;
...
const WindowsOSLogic = struct {
    ...
    const FdSet = extern struct {
        ...
        fn fdSlice(self: *align(8) FdSet) []windows.SOCKET {
            const ptr: [*]u8 = @ptrCast(self);
            const socket_ptr: [*]windows.SOCKET = @alignCast(@ptrCast(ptr + 4 * @sizeOf(c_uint)));
        ...
        fn make(allocator: std.mem.Allocator) !*align(8) FdSet {
            ...
            const mem = try allocator.alignedAlloc(
                u8,
                8,
                4 * @sizeOf(c_uint) + 8 * @sizeOf(windows.SOCKET),
            );
        ...
        fn memSlice(self: *align(8) FdSet) []u8 {
            const ptr: [*]u8 = @ptrCast(self);
            return ptr[0..(4 * @sizeOf(c_uint) + self.capacity * @sizeOf(windows.SOCKET))];
        }
        ...
        fn containsFd(self: *align(8) FdSet, fd: windows.SOCKET) bool {
        ...
        fn addFd(
            fd_set: **align(8) FdSet,
            allocator: std.mem.Allocator,
            new_fd: windows.SOCKET,
        ) !void {
            if (fd_set.*.size == fd_set.*.capacity) {
                // Double our capacity.
                const new_mem_size = 4 * @sizeOf(c_uint) +
                    2 * fd_set.*.capacity * @sizeOf(windows.SOCKET);
                const ptr: []u8 align(8) = @alignCast(fd_set.*.memSlice());
                fd_set.* = @alignCast(@ptrCast((try allocator.reallocAdvanced(
                    ptr,
                    new_mem_size,
                    @returnAddress(),
                )).ptr));
    ...
    read_fds: std.ArrayListUnmanaged(windows.SOCKET),
    write_fds: std.ArrayListUnmanaged(windows.SOCKET),
    ...
    inline fn init(allocator: std.mem.Allocator) !Self {
        const read_fds = std.ArrayListUnmanaged(windows.SOCKET).initCapacity(allocator, 8);
        const write_fds = std.ArrayListUnmanaged(windows.SOCKET).initCapacity(allocator, 8);
    ...
    const Set = enum { read, write, except };
...
/// If `timeout` is not `null`, it describes timeout in nanoseconds until function should return.
/// Note that `timeout` granularity may not be available in nanoseconds and larger granularities
/// are used. If the requested timeout interval requires finer granularity than the implementation
...
pub fn waitForSocketEvent(set: *SocketSet, timeout: ?u64) !usize {
    switch (builtin.os.tag) {
        .windows => {
            ...
            if (read_set == null and write_set == null and except_set == null)
                return 0;
            ...
            return try windows.select(
                0,
                read_set,
                write_set,
                except_set,
                if (timeout != null) &tm else null,
            );
        },
        .linux, .macos, .ios, .watchos, .tvos => return try std.posix.poll(
            ...
            if (timeout) |val| @as(
                i32,
                @intCast((val + std.time.ns_per_ms - 1) / std.time.ns_per_ms),
            ) else -1,
        ),
        else => @compileError("unsupported os " ++ @tagName(builtin.os.tag) ++ " for SocketSet!"),
    }
...
pub fn getEndpointList(allocator: std.mem.Allocator, name: []const u8, port: u16) !*EndpointList {
    const result = blk: {
        ...
        result.* = EndpointList{ .arena = arena, .endpoints = undefined, .canon_name = null };
        break :blk result;
    };
    ...
    if (builtin.link_libc or is_windows) {
        ...
        const freeaddrinfo_fn = if (is_windows)
            windows.funcs.freeaddrinfo
        else
            std.posix.system.freeaddrinfo;
        ...
        while (it) |info| : (it = info.next) {
            ...
            const addr: Address = switch (sockaddr.family) {
                ...
                std.posix.AF.INET6 => block: {
                    ...
                    break :block .{ .ipv6 = Address.IPv6.init(
                        sockaddr_in6.addr,
            ...
            result.endpoints[i] = .{ .address = addr, .port = port };
    ...
    if (builtin.os.tag == .linux) {
        ...
        for (address_list.addrs) |net_addr| {
            const addr: Address = switch (net_addr.any.family) {
                ...
                std.posix.AF.INET6 => .{ .ipv6 = Address.IPv6.init(
                    net_addr.in6.sa.addr,
                    net_addr.in6.sa.scope_id,
                ) },
                else => unreachable,
            };

            result.endpoints[idx] = EndPoint{ .address = addr, .port = port };
            idx += 1;
        }
...
const windows = struct {
    ...
    // TODO: This may be replaceable with `std.posix.socket`'s Windows-specific wrapping.
    fn socket(
        addr_family: u32,
        socket_type: u32,
        protocol: u32,
    ) std.posix.SocketError!ws2_32.SOCKET {
        ...
        //
        // This resolves an issue where "recvfrom" can return a WSAECONNRESET error if a previous
        // send call failed and resulted in an ICMP Port Unreachable message.
    ...
    fn select(
        ...
        while (true) {
            ...
            const result = ws2_32.select(0, read_fds, write_fds, except_fds, timeout);
